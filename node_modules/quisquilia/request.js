...
exports.process = function (message, conference, from, xmpp, rtmp, callback)
{
	...
	if(from == "sum68975687@pvp.net" || from == "Zen Bot Jr") return;
		...
		if(message.indexOf('!') === 0)
		{
			...
			switch(command)
			{
					...
					if(args.length > 0)
					{
						switch(args[0])
						{
							...
							case consts.COMMAND_STATS:
								...
								break;
							case consts.COMMAND_GREETING:
								helpMessage = consts.COMMAND_GREETING_DESCRIPTION;
								break;
							case consts.COMMAND_GREETREMOVE:
								helpMessage = consts.COMMAND_GREETREMOVE_DESCRIPTION;
								break;
						}
					}
				...
				case consts.COMMAND_ADMIN:
					...
					var message;
					if(from == "Broken Chris")
					{
						message = "Cmon Chris dude... be real.";
					}
					else
					{
				...
				case consts.COMMAND_CREATEGAME:
					...
					if(args.length >= 3)
					{
						...
						db.getGame(conference, from, function(game_id)
						{
							...
							{
								callback(to,"Sorry " + from + ", you already have an active game going on.",groupchat);
							}
						});
					}
					else
				...
				case consts.COMMAND_INV:
					...
					break;
				////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
				/// Sets new greeting message for the user
				///
				case consts.COMMAND_GREETING:
					console.log(from + " issued !greeting");
					db.getPermission(from, function(permitted) 
					{
						...
						if (permitted !== null && permitted !== undefined) 
						{
							if (permitted == 1) 
							{
								var greetRaw = args.join(" ");
								var greet = greetRaw.replace("'", "''");

								db.getAccount(from, function(account)
								{
									db.setGreeting(account, greet, function(message)
									{
										if(groupchat) 
										{
											db.getAccount(from, function(account) 
								...
								// db.setGreeting(db.getAccount(from, function(account) 
								// {
								// 	callback(account);
								// }), 
								...
								var message = "You do not have permission to do that.";
								if(groupchat) {
									db.getAccount(from, function(account) {
										xmpp.subscribe(account);
										callback(account, message, false);
				...
				case consts.COMMAND_GREETREMOVE:
					console.log(from + " issued !greetremove");
						var target = args.join(" ");
						if (target.length > 0) {

							var acc = db.getAccount(target, function(account) {
								callback(account);
							});
							console.log(acc);

							if (helper.isAdmin(from)) {
								db.removeGreeting(acc, function(message) {
									if(groupchat) {
										db.getAccount(from, function(account) {
											xmpp.subscribe(account);
											callback(account, message, false);
								...
								var message = "That is an administrator command. Try !greetremove without an argument.";
								if(groupchat) {
									db.getAccount(from, function(account) {
										xmpp.subscribe(account);
										callback(account, message, false);
							...
							var msg = "blahblah";
							var acc = db.getAccount(from, function(account) {
								callback(account);
							});
							console.log(acc);
				...
				case consts.COMMAND_GRANT:
					if (helper.isAdmin(from)) {
						var target = args.join(" ");
						var message = "Permission granted to " + target;
						db.grantPermission(target, function(message){
							if(groupchat) {
								db.getAccount(from, function(account) {
									xmpp.subscribe(account);
									callback(account, message, false);
						...
						var message = "That is an administrator command.";
						if(groupchat) {
							db.getAccount(from, function(account) {
								xmpp.subscribe(account);
								callback(account, message, false);
				...
				case consts.COMMAND_REVOKE:
					if (helper.isAdmin(from)) {
						var target = args.join(" ");
						var message = target + "'s permissions revoked.";
						db.revokePermission(target, function(message){
							if(groupchat) {
								db.getAccount(from, function(account) {
									xmpp.subscribe(account);
									callback(account, message, false);
					...
					case consts.COMMAND_FORTUNECOOKIE:
							db.getConferences(function(conference) {
								var fortuneCookie = consts.FORTUNECOOKIE[Math.floor(Math.random() * (consts.FORTUNECOOKIE.length + 1))];
								xmpp.send(conference, fortuneCookie, true);
							});
					break;
