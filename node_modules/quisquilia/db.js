...
{
	var db = new sqlite3.Database('path_of_database');
	return db;
};
...
exports.setGreeting = function(account, greeting, callback) {
	// escaping greeting - no injection please
	if (account !== undefined && account !== null) {
		var greetingClean = greeting.replace("'","''");
		var query = "UPDATE seen SET greeting = '" + greetingClean + "' WHERE UPPER(account) = '" + account.toUpperCase() + "'";
		var message = "";
		if (greetingClean.length > 0) {
			...
			var query = "SELECT greeting FROM seen WHERE UPPER(account) = '" + account.toUpperCase() + "'";
			var message = "";
			db.get(query, function(err, row) {
...
exports.removeGreeting = function(account, callback) {
	if (account !== undefined && account !== null) {
		var query = "UPDATE seen SET greeting = '' WHERE UPPER(account) = '" + account.toUpperCase() + "'";
		var message = "";
		db.run(query);
